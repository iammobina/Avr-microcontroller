...
; 0000 0027 
; 0000 0028             //input = PINA.0;
; 0000 0029             //if(input == 0)
; 0000 002A             //{
; 0000 002B             //    TIFR = 1<<TOV0;
; 0000 002C             //    PORTA.1 ^=1;
; 0000 002D             //}
; 0000 002E             //else if(input == 1)
; 0000 002F             //{
; 0000 0030             //    count ++;
; 0000 0031             //    if(count == 2)
; 0000 0032             //    {
; 0000 0033             //    TIFR = 1<<TOV0;
; 0000 0034             //    }//0<<OCF0;
; 0000 0035             //}
; 0000 0036 }
	RETI
...
; .FEND
;void Timer1over(){
; 0000 0045 void Timer1over(){
_Timer1over:
; .FSTART _Timer1over
; 0000 0046 
; 0000 0047 int Oneindex;
; 0000 0048     for(Oneindex=0; Oneindex<61; Oneindex++){
	ST   -Y,R17
	ST   -Y,R16
;	Oneindex -> R16,R17
	__GETWRN 16,17,0
_0x4:
	...
	BRGE _0x5
; 0000 0049 
; 0000 004A         TCCR1B=(0<<ICNC1) | (0<<ICES1) | (0<<WGM13) | (0<<WGM12) | (1<<CS12) | (0<<CS11) | (1<<CS10);
	LDI  R30,LOW(5)
	OUT  0x2E,R30
; 0000 004B         TCCR1A=(0<<COM1A1) | (0<<COM1A0) | (0<<COM1B1) | (0<<COM1B0) | (0<<WGM11) | (0<<WGM10);
	LDI  R30,LOW(0)
	OUT  0x2F,R30
; 0000 004C         while((TIFR & (1<<TOV1)==1));
_0x6:
	IN   R30,0x38
	ANDI R30,LOW(0x0)
	BRNE _0x6
...
;
;void Timer0over(){
; 0000 0052 void Timer0over(){
_Timer0over:
; .FSTART _Timer0over
; 0000 0053 int Zeroindex;
; 0000 0054     for(Zeroindex=0; Zeroindex<500; Zeroindex++){
	ST   -Y,R17
	ST   -Y,R16
;	Zeroindex -> R16,R17
	__GETWRN 16,17,0
_0xA:
	__CPWRN 16,17,500
	BRGE _0xB
; 0000 0055         TCCR0=(0<<WGM00) | (0<<COM01) | (0<<COM00) | (0<<WGM01) | (1<<CS02) | (0<<CS01) | (0<<CS00);
	LDI  R30,LOW(4)
	OUT  0x33,R30
; 0000 0056         while((TIFR & (1<<TOV0) == 1));
_0xC:
	IN   R30,0x38
	SBRC R30,0
	RJMP _0xC
...
; 0000 0060 char input_port;
; 0000 0061 // Input/Output Ports initialization
; 0000 0062 // Port A initialization
; 0000 0063 // Function: Bit7=In Bit6=In Bit5=In Bit4=In Bit3=In Bit2=In Bit1=Out Bit0=In
; 0000 0064 DDRA=(0<<DDA7) | (0<<DDA6) | (0<<DDA5) | (0<<DDA4) | (0<<DDA3) | (0<<DDA2) | (1<<DDA1) | (0<<DDA0);
;	input_port -> R17
	LDI  R30,LOW(2)
	OUT  0x1A,R30
; 0000 0065 // State: Bit7=T Bit6=T Bit5=T Bit4=T Bit3=T Bit2=T Bit1=0 Bit0=T
...
; 0000 006A DDRB=(0<<DDB7) | (0<<DDB6) | (0<<DDB5) | (0<<DDB4) | (0<<DDB3) | (0<<DDB2) | (0<<DDB1) | (0<<DDB0);
	OUT  0x17,R30
...
; 0000 007C // Clock value: 62.500 kHz
; 0000 007D // Mode: Normal top=0xFF
; 0000 007E // OC0 output: Disconnected
; 0000 007F // Timer Period: 4.096 ms
; 0000 0080 TCCR0=(0<<WGM00) | (0<<COM01) | (0<<COM00) | (0<<WGM01) | (1<<CS02) | (0<<CS01) | (0<<CS00);
	LDI  R30,LOW(4)
	OUT  0x33,R30
...
; 0000 0086 // Clock value: 62.500 kHz
...
; 0000 008C // Timer Period: 1.0486 s
; 0000 008D // Timer1 Overflow Interrupt: On
...
; 0000 0091 TCCR1A=(0<<COM1A1) | (0<<COM1A0) | (0<<COM1B1) | (0<<COM1B0) | (0<<WGM11) | (0<<WGM10);
	OUT  0x2F,R30
; 0000 0092 TCCR1B=(0<<ICNC1) | (0<<ICES1) | (0<<WGM13) | (0<<WGM12) | (1<<CS12) | (0<<CS11) | (0<<CS10);
	LDI  R30,LOW(4)
	OUT  0x2E,R30
...
; 0000 009D TIMSK=(0<<OCIE2) | (0<<TOIE2) | (0<<TICIE1) | (0<<OCIE1A) | (0<<OCIE1B) | (1<<TOIE1) | (0<<OCIE0) | (1<<TOIE0);
	LDI  R30,LOW(5)
	OUT  0x39,R30
; 0000 009E 
...
; 0000 00A5       input_port = 0b00000001;
	LDI  R17,LOW(1)
; 0000 00A6       if((PINA & input_port) == 1 ){
	IN   R30,0x19
	AND  R30,R17
	CPI  R30,LOW(0x1)
	BRNE _0x12
; 0000 00A7       Timer0over();
	RCALL _Timer0over
; 0000 00A8       }
; 0000 00A9       else{
	RJMP _0x13
_0x12:
; 0000 00AA       Timer1over();
	RCALL _Timer1over
; 0000 00AB       }
_0x13:
; 0000 00AC       PORTA.1 ^= 1;
